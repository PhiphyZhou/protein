...
class Seq2SeqModel(object):
  ...
  def __init__(self, feature_size, buckets, size,
    ...
    Args:
      feature_size: dimension of the frame.
    ...
    if num_samples > 0 and num_samples < self.feature_size:
      with tf.device("/cpu:0"):
        w = tf.get_variable("proj_w", [size, self.feature_size])
        w_t = tf.transpose(w)
        b = tf.get_variable("proj_b", [self.feature_size])
    ...
    def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
      return seq2seq.embedding_attention_seq2seq(
          encoder_inputs, decoder_inputs, cell, feature_size,
          feature_size, output_projection=output_projection,
